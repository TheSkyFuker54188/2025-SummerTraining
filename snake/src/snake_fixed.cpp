...
struct Item { Point pos; int value, lifetime; };
struct Snake {
...
struct Key { Point pos; int holder_id, remaining_time; };
struct SafeZoneBounds { int x_min, y_min, x_max, y_max; };
struct Target { Point pos; int value, distance; double priority; };
...
private:
  ...
  void analyze_danger(const GameState& state) {
    ...
    // 收缩区域标记
    if (state.next_shrink_tick > 0) mark_shrinking_areas(state);
  }
...
public:
  ...
  int danger(const Point& p) const {
    if (p.y < 0 || p.y >= MAXM || p.x < 0 || p.x >= MAXN) return EXTREME_DANGER;
    return danger_map[p.y][p.x];
  }
  
  int value(const Point& p) const {
    if (p.y < 0 || p.y >= MAXM || p.x < 0 || p.x >= MAXN) return 0;
    return value_map[p.y][p.x];
  }
  
  bool is_safe(const Point& p) const {
    if (p.y < 0 || p.y >= MAXM || p.x < 0 || p.x >= MAXN) return false;
    return danger_map[p.y][p.x] < DANGER_THRESHOLD;
  }
  
  // 综合评分 - 结合三个维度
  float score(const Point& p) const {
    if (p.y < 0 || p.y >= MAXM || p.x < 0 || p.x >= MAXN) return -1000.0f;
    if (danger_map[p.y][p.x] >= HIGH_DANGER) return -500.0f;
...
private:
  ...
  Target select_target(const GameState& state) {
    ...
    for (int y = 0; y < MAXM; y++) {
      for (int x = 0; x < MAXN; x++) {
        ...
        float dist_factor = (distance <= NEAR_FOOD_THRESHOLD) ? 
                           NEAR_FOOD_FACTOR / (distance + 1) : FAR_FOOD_FACTOR / distance;
        
        Target t;
        t.pos = p;
        t.value = pos_value;
        t.distance = distance;
        t.priority = map->score(p) * dist_factor;
        targets.push_back(t);
      }
    }
    ...
    return {{(state.current_safe_zone.y_min + state.current_safe_zone.y_max) / 2,
             (state.current_safe_zone.x_min + state.current_safe_zone.x_max) / 2}, 0, 0, 0};
  }
  ...
  float evaluate_direction(int direction, const GameState& state, const Target& target) {
    ...
    if (!is_direction_safe(direction, state)) return -1000.0f;
  ...
  bool is_direction_safe(int direction, const GameState& state) {
    ...
    for (size_t i = 1; i < self.body.size(); i++) {
      if (next.x == self.body[i].x && next.y == self.body[i].y) return false;
    }
  ...
  bool should_use_shield(const GameState& state) {
    const auto& self = state.get_self();
    if (self.shield_cd > 0 || self.score < SHIELD_ACTIVATION_COST || self.shield_time > 0) return false;
...
public:
  ...
  int make_decision(const GameState& state) {
    ...
    // 检查护盾使用
    if (should_use_shield(state)) return SHIELD_COMMAND;
    
    // 选择最佳方向
    for (const auto& [dir, score] : direction_scores) {
      if (score > 0) return dir;
    }
...
void read_game_state(GameState& s) {
  ...
  cin >> s.next_shrink_tick >> s.next_safe_zone.x_min >> s.next_safe_zone.y_min 
      >> s.next_safe_zone.x_max >> s.next_safe_zone.y_max;
  cin >> s.final_shrink_tick >> s.final_safe_zone.x_min >> s.final_safe_zone.y_min 
      >> s.final_safe_zone.x_max >> s.final_safe_zone.y_max;
}
